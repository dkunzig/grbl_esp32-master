...
#include "c:\grbl_esp32-master\main\serial2socket.h"
//void * Serial2Socket;
...
bool WebSocketsServer::checkIfConnected(int num)
{
	try
	{
		WSclient_t * client = &_clients[num];
...
void WebSocketsServer::messageReceived(WSclient_t * client, WSopcode_t opcode, uint8_t * payload, size_t length, bool fin) {
    ...
    switch(opcode) {
        case WSop_text:
			...
			Serial2Socket.write_to_GRBL(payload);
            break;
        case WSop_binary:
...
//void attachSerial2socket(void *_Serial2Socket)
//{
//	if (_Serial2Socket) {
//		Serial2Socket = _Serial2Socket;
//		return true;
//}
...
bool WebSocketsServer::clientIsConnected(WSclient_t * client) {
    ...
    if(client->tcp->connected()) {
		...
		printf("in client connected tcp\n\n");
        // client lost
        if(client->status != WSC_NOT_CONNECTED) {
