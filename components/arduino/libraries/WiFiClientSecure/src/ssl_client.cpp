...
#include "WiFi.h"
#include "C:\Grbl_Esp32-master\main\ssl.h"
...
int start_ssl_client(sslclient_context *ssl_client, const char *host, uint32_t port, const char *rootCABuff, const char *cli_cert, const char *cli_key, const char *pskIdent, const char *psKey)
{
    ...
    if (rootCABuff != NULL) {
        ...
        // set mbedtls config
        //ret = mbedtls_ssl_conf_psk(&ssl_client->ssl_conf, psk, psk_len, removed don
        //         (const unsigned char *)pskIdent, strlen(pskIdent));    removed don
		ret = 0;
        if (ret != 0) {
