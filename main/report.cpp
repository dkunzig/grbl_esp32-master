...
{	
	...
	if (webattached)
	{
		if (((WebSocketsServer *)websocket)->checkIfConnected(0))
		{
			((WebSocketsServer *)websocket)->sendTXT(0, text);
			//printf("Text sent text = %s\n", text);
		}
	}
}
...
{	
  switch(status_code) {
	  printf("report status message client = %d\n",client);
