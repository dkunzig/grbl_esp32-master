...
void HomeMotorX()
{
	digitalWrite(X_DIRECTION_PIN, 0);
	//printf("Enter HomeMotorX\n");
	while (1)
	{
		if (digitalRead(X_LIMIT_PIN) == 1)
		{
			//MotorStepHigh;
...
void HomeMotorY()
{
	digitalWrite(Y_DIRECTION_PIN, 0);
	digitalWrite(Y_DIRECTION_B_PIN, 0);

	//printf("Enter HomeMotorY\n");
	while (1)
	{
		if (digitalRead(Y_LIMIT_PIN) == 1)
		{
			//MotorStepHigh;
		...
		if (digitalRead(Y_LIMIT_B_PIN) == 1)
		{
			//MotorStepHigh;
			digitalWrite(Y_STEP_B_PIN, 1);
			esp_delay_us(15);
			//MotorStepLow;
			digitalWrite(Y_STEP_B_PIN, 0);
			esp_delay_us(400);
		}
		if (digitalRead(Y_LIMIT_PIN) == 0 && digitalRead(Y_LIMIT_B_PIN) == 0)
			break;
	}
}
...
void BackOff_All_Motors()
{
	...
	digitalWrite(Y_DIRECTION_PIN, 1);
	digitalWrite(Y_DIRECTION_B_PIN, 1);
	digitalWrite(X_DIRECTION_PIN, 1);
	printf("z limit pin = %d\n", digitalRead(Z_LIMIT_PIN));
	printf("Y limit pin = %d\n", digitalRead(Y_LIMIT_PIN));
	printf("Y limit B pin = %d\n", digitalRead(Y_LIMIT_B_PIN));
	printf("X limit pin = %d\n", digitalRead(X_LIMIT_PIN));
	...
	while (1)
	{
		...
		if (digitalRead(Y_LIMIT_B_PIN) == 0)
		{
			//MotorStepHigh;
			digitalWrite(Y_STEP_B_PIN, 1);
			esp_delay_us(15);
		...
		if (digitalRead(Y_LIMIT_PIN) == 1 && digitalRead(Y_LIMIT_B_PIN) == 1 && digitalRead(Z_LIMIT_PIN) == 1 && digitalRead(X_LIMIT_PIN) == 1)
			break;
	}
	int idx;
...
void MoveMotorA()
{
	digitalWrite(A_DIRECTION_PIN, 1);
	//printf("Enter MoveMotorA\n");
	int i = 0;
	for(i=0;i < 2000; i++)
	{
		//MotorStepHigh;
		digitalWrite(A_STEP_PIN, 1);
		esp_delay_us(15);
		//MotorStepLow;
		digitalWrite(A_STEP_PIN, 0);
		esp_delay_us(800);
	}
	digitalWrite(A_DIRECTION_PIN,0);
...
uint8_t limits_get_state()
{
	...
	#endif
	#ifdef Y_LIMIT_B_PIN
			pin |= (digitalRead(Y_LIMIT_B_PIN) << 3);
